...
def ef_risk_aipw(theta, delta, s, a, c,
                 ...
                 unique_censor_times, all_unique_event_times, strata_unique_event_times,
                 time_matrix_s1=None, final_time_matrix_s1=None, risk_set_matrix_s1=None,
                 strata_s1_censor_times=None,
                 time_matrix_s0=None, final_time_matrix_s0=None, risk_set_matrix_s0=None,
                 strata_s0_censor_times=None):
    alpha = theta[:n_unique_times]
    beta = theta[n_unique_times:]
    beta_all = theta[n_unique_times:]
    # TODO gamma = theta[]
    ...
    # IPW model
    ef_ipw = ef_weight_models(theta, s=s, a=a, c=c,
                              sample_matrix=sample_matrix, action_matrix=action_matrix, censor_matrix=None,
                              time_matrix_s1=time_matrix_s1, final_time_matrix_s1=final_time_matrix_s1,
                              risk_set_matrix_s1=risk_set_matrix_s1,
                              time_matrix_s0=time_matrix_s0, final_time_matrix_s0=final_time_matrix_s0,
                              risk_set_matrix_s0=risk_set_matrix_s0)
    ipw = construct_weights(theta=beta_all, s=s, a=a,
                            sample_matrix=sample_matrix, action_matrix=action_matrix, censor_matrix=censor_matrix,
                            time_matrix_s1=time_matrix_s1, strata_s1_times=strata_s1_censor_times,
                            time_matrix_s0=time_matrix_s0, strata_s0_times=strata_s0_censor_times,
                            unique_censor_times=unique_censor_times,
                            unique_event_times=strata_unique_event_times)

    # Pooled logistic estimation
    ef_plogit = ef_pooled_logit(theta=beta,
                                delta=delta,
                                baseline_matrix=baseline_matrix,
                                time_matrix=time_matrix,
                                final_time_matrix=final_time_matrix,
    ...
    risks = plogit_predictions(theta=beta,
                               baseline_matrix=baseline_matrix,
                               time_matrix=time_matrix,
                               strata_unique_times=strata_unique_event_times,
                               all_unique_times=all_unique_event_times)
    ef_risks = (risks - np.asarray(alpha)[:, None]) * s

    # Returning stacked estimating equations
    return np.vstack([ef_risks, ef_ipw, ef_plogit])
