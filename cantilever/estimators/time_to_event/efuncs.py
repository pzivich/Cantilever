...
# TODO diagnostic are incorrect! They don't incorporate the distance in time between jumps
...
def ef_weight_models(theta, s, a, sample_matrix, action_matrix):
...
def construct_weights(theta, s, a, sample_matrix, action_matrix):
    m_index1 = sample_matrix.shape[1]
...
def ef_product_limit(theta, delta, final_time_matrix, risk_set_matrix, contributions, weights=None):
    # TODO need to check compatible with time-varying weights...
...
def ef_risk_product_limit(theta, s, a, delta, sample_matrix, action_matrix,
    ...
    ee_weights = ef_weight_models(theta=beta_all, s=s, a=a, sample_matrix=sample_matrix, action_matrix=action_matrix)
    ipw = construct_weights(theta=beta_all, s=s, a=a, sample_matrix=sample_matrix, action_matrix=action_matrix)
...
def ef_diagnostic_product_limit(theta, s, a, delta, sample_matrix, action_matrix,
                                final_time_matrix, risk_set_matrix,
                                contribute_s1, contribute_s0):
    # TODO doesn't work because need to call product-limit estimator, not generate risk functions like g-comp...
    #   duh!
    n_unique_times = risk_set_matrix.shape[0]
    obs_n = delta.shape[0]
    ird = theta[0]
    ...
    ee_risk1 = ef_product_limit(theta=risk1,
                                ...
                                final_time_matrix=final_time_matrix,
                                risk_set_matrix=risk_set_matrix,
                                contributions=contribute_s1,
                                weights=ipw)
    ee_risk0 = ef_product_limit(theta=risk0,
                                delta=delta,
                                final_time_matrix=final_time_matrix,
                                risk_set_matrix=risk_set_matrix,
                                contributions=contribute_s0,
                                weights=ipw)

    # Diagnostic risk function estimation
    ee_diag = ((risk1 - risk0) - np.asarray(risk_diagnostic))[:, None] * np.ones(obs_n)
    ee_ird = (ird - np.sum(risk1)) * np.ones(obs_n)
    # Returning stacked estimating equations
    return np.vstack([ee_ird, ee_diag, ee_risk1, ee_risk0, ee_weights])


def ef_diagnostic_product_limit_show(theta, s, a, delta, sample_matrix, action_matrix,
                                     final_time_matrix, risk_set_matrix, contribute_s1, contribute_s0):
    n_unique_times = risk_set_matrix.shape[0] + 1
    obs_n = delta.shape[0]
    ird = theta[0]
    alpha = theta[1:n_unique_times]
    beta_all = theta[n_unique_times:]
    ...
    # Estimating diagnostic function via product limit
    risk1 = product_limit_predict(delta=delta, final_time_matrix=final_time_matrix,
                                  risk_set_matrix=risk_set_matrix, contributions=contribute_s1,
                                  weights=ipw)
    risk0 = product_limit_predict(delta=delta, final_time_matrix=final_time_matrix,
                                  risk_set_matrix=risk_set_matrix, contributions=contribute_s0,
                                  weights=ipw)
    print(risk1)
    print(risk0)
...
def ef_diagnostic_function(theta, delta, baseline_matrix, sample,
    ...
    ee_ird = (ird - np.sum(alpha)) * np.ones(obs_n)
    # Returning stacked estimating equations
